...
public class Car implements Comparable<Car>{
    ...
    public int getStartTime() {
        if (!this.isPreset)//非预置车辆返回localTime
            return localtime;
        return startTime;
    }
...
//        road = null;
    ...
    public boolean roadPlanning(Map map, int s, TreeMap<Vector, Road> roadST) {//s表示起点路口ID,规划成功返回true
        Stack<Road> planingpath = new Stack<>();//清空
        double weight;
        Road[] edgeTo;
        double[] distTo;
        IndexMinPQ<Double> pq;
        edgeTo = new Road[map.V() + 1];
        distTo = new double[map.V() + 1];
        pq = new IndexMinPQ<>(map.V());
        for (int v = 0; v < distTo.length; v++) {
            distTo[v] = Double.POSITIVE_INFINITY;
        }
        distTo[s] = 0.0;
        pq.insert(s, 0.0);
        if (road != null) {//不是从始发地规划
            Road r = road.brothRoad(roadST);
            if (road.brothRoad(roadST) != null) {
                weight = road.brothRoad(roadST).getWeight();//保护权值
                road.brothRoad(roadST).setWeight(Double.POSITIVE_INFINITY);//禁止掉头

                while (!pq.isEmpty()) {
                    relax(map, edgeTo, distTo, pq);
                }
                for (Road e = edgeTo[destination]; e != null; e = edgeTo[e.from()]) {
                    planingpath.push(e);
                }
                road.brothRoad(roadST).setWeight(weight);//恢复权值
            }else {
                while (!pq.isEmpty()) {
                    relax(map, edgeTo, distTo, pq);
                }
            ...
            if (!planingpath.isEmpty()) {
                planPath = planingpath;
                return true;
