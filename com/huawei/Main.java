...
public class Main {
    ...
    public static void main(String[] args){
        ...
        double q;
        if (map.capacity() < 35000)
            q = 0.146;
        else
            q = 0.132;
        int maxNum = (int) (map.capacity() * q);//设定地图最多能跑的车数
        calculate(carST, map, roadST);
        Scheduler scheduler = new Scheduler();
        scheduler.setMaxCarNum(maxNum);

        int time = 0;
        while (true) {
            /***********************/
            scheduler.relaxRoad(roadST, map, 0.0);//引流
            ...
            for (Vector v : roadST.keySet()//根据交通状况重设权值
            ) {
                Road road= roadST.get(v);
                road.initializeWeight();
                road.clearInitTree();//清空道路上车序列
            }
            Car car = null;
            ...
            while (iter.hasNext()) {
                ...
                car.addTime();
                car.initialCarState();
                if (!car.isRunning()){//车库中的车
                    if (car.getLocaltime() >= car.getplaneTime()) {
//                        car.roadPlanning(map, car.getDestination(), roadST);
                        car.nextRoad().addCarInInitTree(car);
                    }
                }
            }
        ...
        fileInterface.writeAnswer(arrivedCar, answerPath);
    }
    ...
    private static void calculate(TreeMap<Integer, Car> carST, Map map, TreeMap<Vector, Road> roadST){
        ...
        for (int carid :
            ...
            if (!car.isPreset())
                car.roadPlanning(map, car.getOrigin(), roadST);
        }
